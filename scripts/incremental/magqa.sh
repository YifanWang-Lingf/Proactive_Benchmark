...
do
  ...
  do
    ...
    logfile="outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec.log.$i"
    ...
    CUDA_VISIBLE_DEVICES=${GPUS[i]} python -u inference_incremental_chunks.py \
        ...
        --video_chunk_sec ${video_chunk_sec} \
        --test_fname ../qa_generate/outputs/magqa/0502_version-anno.json \
        --output_fname outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec.jsonl.$i \
    |& tee -a "$logfile"      
  ...
  cat outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec.jsonl.* \
      > outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec.jsonl
  ...
  python reformat_to_mmduet_format.py \
      --input_fname  outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec.jsonl \
      --output_fname outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec-pred.jsonl \
  |& tee -a "outputs/${model_dirname}/magqa/incremental/${video_chunk_sec}sec.reformat.log"
...
# GPUS=(2 2)
# interval=500
# mkdir -p outputs/llava-ov/magqa/incremental
# for video_chunk_sec in 2 # 3 4
# do
#     # for i in 0 1
#     # do
...
#     # --input_dir /share2/wangyq/projects/MMDuet/datasets/shot2story/videos --frame_fps 2 --max_num_frames 400 \
#     # --video_chunk_sec ${video_chunk_sec} \
#     # --test_fname ../judge_questions/outputs/magqa/magqa_test-with_checked_judge_questions-1000_examples.json \
#     # --output_fname outputs/llava-ov/magqa/incremental//${video_chunk_sec}sec-no_new_line.jsonl.$i \
#     # > outputs/llava-ov/magqa/incremental/${video_chunk_sec}sec-no_new_line.log.$i 2>&1 &
#     # done
#     # wait

#     # cat outputs/llava-ov/magqa/incremental/${video_chunk_sec}sec.jsonl.* > outputs/llava-ov/magqa/incremental/${video_chunk_sec}sec.jsonl

#     # python reformat_to_mmduet_format.py \
#     # --input_fname outputs/llava-ov/magqa/incremental/${video_chunk_sec}sec.jsonl \
#     # --output_fname outputs/llava-ov/magqa/incremental/${video_chunk_sec}sec-pred.jsonl
# done
# } &