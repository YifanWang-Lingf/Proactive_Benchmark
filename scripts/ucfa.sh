...
do
  ...
  do
    ...
    logfile="outputs/${model_dirname}/ucfa/${video_chunk_sec}sec.log.$i"
    ...
    CUDA_VISIBLE_DEVICES=${GPUS[i]} python -u inference_independent_chunks.py \
        ...
        --video_chunk_sec ${video_chunk_sec} \
        --test_fname ../qa_generate/outputs/ucfa/0502_version-anno.json \
        --output_fname outputs/${model_dirname}/ucfa/${video_chunk_sec}sec.jsonl.$i \
    |& tee -a "$logfile"      
  ...
  cat outputs/${model_dirname}/ucfa/${video_chunk_sec}sec.jsonl.* \
      > outputs/${model_dirname}/ucfa/${video_chunk_sec}sec.jsonl
  ...
  python reformat_to_mmduet_format.py \
      --input_fname  outputs/${model_dirname}/ucfa/${video_chunk_sec}sec.jsonl \
      --output_fname outputs/${model_dirname}/ucfa/${video_chunk_sec}sec-pred.jsonl \
  |& tee -a "outputs/${model_dirname}/ucfa/${video_chunk_sec}sec.reformat.log"
